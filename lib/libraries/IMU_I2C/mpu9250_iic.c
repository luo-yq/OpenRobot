...
u8 MPU9250_Init(void)
{
    ...
    GPIO_Initure.Pin=GPIO_PIN_4|GPIO_PIN_6; //PB0,1
    GPIO_Initure.Mode=GPIO_MODE_OUTPUT_PP;  //推挽输出
    GPIO_Initure.Pull=GPIO_PULLUP;          //上拉
    GPIO_Initure.Speed=GPIO_SPEED_HIGH;     //高速
    HAL_GPIO_Init(GPIOA,&GPIO_Initure);     //初始化GPIOB.0和GPIOB.1
	
	HAL_GPIO_WritePin(GPIOA,GPIO_PIN_4,GPIO_PIN_SET);	//PB1置1
    HAL_GPIO_WritePin(GPIOA,GPIO_PIN_6,GPIO_PIN_RESET);	//PB1置1
    ...
    res=MPU_Read_Byte(0x68,0x75);  //读取MPU6500的ID
